...
ols <- summary(model_ols)

# Uji F #
summary(model_ols)
ols$fstatistic[1]
pf(ols$fstatistic[1], ols$fstatistic[2], ols$fstatistic[3], lower.tail = FALSE)

# Uji t #
coeftest(model_ols)
round(ols$coefficients[, c("t value", "Pr(>|t|)")], 4)

# Uji Wald #
library(car)

linearHypothesis(model_ols, "(Intercept) + D = 0")
linearHypothesis(model_ols, "X1 + X1D = 0")
linearHypothesis(model_ols, "X2 + X2D = 0")
linearHypothesis(model_ols, "X3 + X3D = 0")
linearHypothesis(model_ols, "X4 + X4D = 0")
linearHypothesis(model_ols, "X5 + X5D = 0")
linearHypothesis(model_ols, "X6 + X6D = 0")
linearHypothesis(model_ols, "X7 + X7D = 0")
linearHypothesis(model_ols, "X8 + X8D = 0")