...
.welcome-screen {
  ...
  width: 100%;
  height: 100%;
  background-image: url('https://lh3.googleusercontent.com/proxy/mm4fMeB9RNIGB6y0I9Moc2_M1pXh1tqGbKG56IhYFeIldBGbF1pEp2dbbUxpa7rd49tNZz6IHpoQ7KUngn6uUxTtmkkGdwiGB1uCBXD86uSXCOOInH16vbtNnvuSCllD_iaqD9tIlEPy_pq8Sl2U4RxA8kbBr0uR1drktl1eJJo-W22JQBxUKOH1cJDoCLt9Pnt2ixqvpiM');
...
sidebar <- dashboardSidebar(
  ...
  box(
    ...
    # Status indicator
    uiOutput("spatial_clustering_status_ui")
  ),
...
body <- dashboardBody(
  ...
  div(
    ...
    div(
      class = "welcome-card",
      tags$img(
        src = "https://upload.wikimedia.org/wikipedia/id/thumb/6/62/Logo_Unpad.JPG/500px-Logo_Unpad.JPG",
        class = "welcome-logo"
      ),
      h2("Analisis Tingkat Kemiskinan di Provinsi Jawa Barat Menggunakan Metode Spatial Clustering Regression", class = "welcome-title"),
      h4("© 2026 SCR Dashboard", class = "welcome-subtitle"),
      
      div(class = "welcome-divider"),
      
      h4("Developed By:", style = "color: #2980b9; font-weight: bold;"),
      div(class = "welcome-developers",
          p("Theodora Clara"),
      ...
      div(class = "welcome-institution",
          p("Department of Statistics"),
          p("Faculty of Mathematics and Natural Sciences"),
          p("Universitas Padjadjaran"),
      ),
      
      actionButton(
        "enter_dashboard",
        HTML("Masuk ke Dashboard →"),
      ...
      tabItem(
        ...
        fluidRow(
          column(
            width = 12,
            box(
              title       = "Spatial Clustering Polygon Map",
              width       = 12,
              status      = "success",
              solidHeader = TRUE,
              collapsible = TRUE,
              ...
              tags$div(
                ...
                icon("info-circle"),
                " Arahkan kursor atau klik pada poligon untuk melihat detail wilayah."
              )
            ),
        ...
        # TAB BOX FOR SPATIAL CLUSTERING ANALYSIS
        fluidRow(
          column(
            width = 12,
            tabBox(
              ...
              # TAB 1: PENGUJIAN AUTOKORELASI SPASIAL
              tabPanel(
                ...
                fluidRow(
                  column(
                    ...
                    box(
                      width = 12,
                      status = "warning",
                      title = "Spatial Weight Matrix Summary",
                      solidHeader = TRUE,
                      verbatimTextOutput("spatial_weight_summary")
                    )
                  )
                ),
                fluidRow(
                  column(
                    ...
                    box(
                      width = 12,
                      status = "info",
                      title = "Neighbors Check",
                      solidHeader = TRUE,
                      collapsible = TRUE,
                      verbatimTextOutput("neighbors_check")
                    )
                  )
                )
              ),
              
              # TAB 2: MATRIKS JARAK EUCLIDEAN
              tabPanel(
                ...
                fluidRow(
                  column(
                    ...
                    box(
                      ...
                      solidHeader = TRUE,
                      DTOutput("euclidean_distance_matrix")
                    )
                  )
                ),
                fluidRow(
                  column(
                    ...
                    box(
                      width = 12,
                      status = "success",
                      title = "Distance Summary Statistics",
                      solidHeader = TRUE,
                  ...
                  column(
                    ...
                    box(
                      ...
                      solidHeader = TRUE,
                      plotOutput("distance_distribution", height = "300px")
                    )
                  )
                )
              ),
              
              # TAB 3: MATRIKS PEMBOBOT SPASIAL
              tabPanel(
                ...
                fluidRow(
                  column(
                    ...
                    box(
                      width = 12,
                      status = "info",
                      title = "Unstandardized Contiguity Matrix",
                      solidHeader = TRUE,
                      DTOutput("binary_weight_matrix")
                    )
                  ),
                  column(
                    width = 6,
                    box(
                      width = 12,
                      status = "info",
                      title = "Standardized Weight Matrix",
                      solidHeader = TRUE,
                      DTOutput("standardized_weight_matrix")
                    )
                  )
                ),
                fluidRow(
                  column(
                    ...
                    box(
                      width = 12,
                      status = "warning",
                      title = "Spatial Weights Characteristics",
                      solidHeader = TRUE,
                      verbatimTextOutput("weights_characteristics")
                    )
                  )
                )
              ),
              
              # TAB 4: NILAI PARAMETER ALPHA CHIC
              tabPanel(
                "Parameter Alpha CHIC",
                fluidRow(
                  column(
                    width = 6,
                    box(
                      width = 12,
                      status = "primary",
                      title = "Choice Alpha Results",
                      solidHeader = TRUE,
                      DTOutput("alpha_choice_table")
                    )
                  ),
                  column(
                    width = 6,
                    box(
                      width = 12,
                      status = "primary",
                      title = "Normalized Criterion Plot",
                      solidHeader = TRUE,
                      plotOutput("alpha_choice_plot", height = "400px")
                    )
                  )
                ),
                fluidRow(
                  column(
                    ...
                    box(
                      width = 12,
                      status = "success",
                      title = "Optimal Alpha Selection",
                      solidHeader = TRUE,
                      verbatimTextOutput("alpha_recommendation")
                    )
                  )
                )
              ),
              
              # TAB 5: GRAPH SILHOUETTE
              tabPanel(
                "Graph Silhouette",
                fluidRow(
                  column(
                  ...
                  column(
                    ...
                    box(
                      width = 12,
                      status = "info",
                      title = "Silhouette Results",
                      solidHeader = TRUE,
                      DTOutput("silhouette_table")
                    )
                  )
                ),
                fluidRow(
                  column(
                    ...
                    box(
                      ...
                      solidHeader = TRUE,
                      uiOutput("optimal_cluster_info")
                    )
                  )
                )
              ),
              
              # TAB 6: DENDROGRAM SPATIAL CLUSTERING
              tabPanel(
                "Results",
                fluidRow(
                  column(
                    ...
                    box(
                      ...
                      title = "Dendrogram",
                      solidHeader = TRUE,
                      plotOutput("spatial_dendrogram", height = "500px")
                    )
                  ),
                  column(
                    width = 6,
                    box(
                      ...
                      title = "Map",
                      solidHeader = TRUE,
                      leafletOutput("cluster_result_map", height = "500px")
                    )
                  )
                )
              )
            ...
            box(
              ...
              tags$div(
                ...
                tags$ul(
                  ...
                  tags$li("Complete spatial clustering analysis: Moran's I, distance matrices, alpha CHIC, silhouette, and dendrogram")
                ),
                br(),
                ...
                tags$ul(
                  ...
                  tags$li("Spatial autocorrelation testing (Moran's I)"),
                  tags$li("Hierarchical clustering with spatial constraints (ClustGeo)"),
                  tags$li("Silhouette analysis for optimal cluster determination")
                ),
                br(),
                h4("Developed by:"),
                p("Theodora Clara"),
...
server <- function(input, output, session) {
  ...
  # Reactive values untuk spatial clustering
  spatial_clustering <- reactiveValues(
    nb = NULL,
    wc = NULL,
    ww = NULL,
    w_listw = NULL,
  ...
  observeEvent(input$enter_dashboard, {
    shinyjs::hide("welcome-screen")
    shinyjs::show("main-dashboard")
  })
  ...
  # Fungsi untuk membersihkan geometri yang invalid
  clean_geometry <- function(sf_object) {
    tryCatch({
      # Cek apakah geometri valid
      invalid <- !st_is_valid(sf_object)
      
      if(any(invalid)) {
        showNotification(
          paste("Memperbaiki", sum(invalid), "geometri yang invalid..."),
          type = "warning",
          duration = 3
        )
        
        # Perbaiki geometri yang invalid
        sf_object <- st_make_valid(sf_object)
      }
      
      # Hapus duplikat vertex (kalau ada)
      sf_object <- st_simplify(sf_object, preserveTopology = TRUE, dTolerance = 0)
      
      return(sf_object)
      
    }, error = function(e) {
      showNotification(
        paste("Error membersihkan geometri:", e$message),
        type = "error",
        duration = 5
      )
      return(sf_object)
  ...
  # Fungsi untuk menghitung spatial clustering ketika shapefile diupload
  observeEvent(values$shape_data, {
    req(values$shape_data)
    
    # Reset status
    spatial_clustering$data_ready <- FALSE
    spatial_clustering$error_message <- NULL
    
    # Bersihkan geometri terlebih dahulu
    data_clean <- clean_geometry(values$shape_data)
    
    tryCatch({
      # Validasi kolom TngktKm
      if(!"TngktKm" %in% names(data_clean)) {
        spatial_clustering$error_message <- "Kolom 'TngktKm' tidak ditemukan dalam data shapefile"
        showNotification(spatial_clustering$error_message, type = "error", duration = 5)
        return(NULL)
      }
      
      # Hapus missing values jika ada
      data_clean <- data_clean[!is.na(data_clean$TngktKm), ]
      
      if(nrow(data_clean) < 3) {
        spatial_clustering$error_message <- "Jumlah observasi terlalu sedikit (minimal 3)"
        showNotification(spatial_clustering$error_message, type = "error", duration = 5)
        return(NULL)
      }
      
      showNotification("Memproses spatial clustering...", type = "default", duration = 2)
      
      # 1. Membuat matriks pembobot spasial 
      spatial_clustering$nb <- tryCatch({
        poly2nb(data_clean, queen = TRUE)
      }, error = function(e) {
        showNotification("Mencoba dengan rook contiguity...", type = "warning", duration = 2)
        poly2nb(data_clean, queen = FALSE)
      })
      
      # Cek apakah ada region tanpa neighbors
      no_neighbors <- which(card(spatial_clustering$nb) == 0)
      if(length(no_neighbors) > 0) {
        showNotification(
          paste("Warning:", length(no_neighbors), "region tidak memiliki neighbors"),
          type = "warning",
          duration = 3
        )
      }
      
      # Buat weight matrices dengan zero.policy = TRUE
      spatial_clustering$wc <- nb2mat(spatial_clustering$nb, style = 'B', zero.policy = TRUE)
      spatial_clustering$ww <- nb2mat(spatial_clustering$nb, style = 'W', zero.policy = TRUE)
      spatial_clustering$w_listw <- nb2listw(spatial_clustering$nb, zero.policy = TRUE)
      
      # 2. Pengujian autokorelasi spasial dengan zero.policy = TRUE
      spatial_clustering$moran_test <- tryCatch({
        moran.test(data_clean$TngktKm, 
                   spatial_clustering$w_listw, 
                   zero.policy = TRUE,
                   na.action = na.exclude)
      }, error = function(e) {
        showNotification(paste("Moran's I test error:", e$message), type = "warning", duration = 3)
        NULL
      })
      
      # 3. Matriks jarak Euclidean (D0)
      spatial_clustering$d0 <- dist(data_clean$TngktKm)
      
      # 4. Matriks pembobot spasial (D1) 
      if(!is.null(spatial_clustering$wc)) {
        spatial_clustering$d1 <- as.dist(1 - spatial_clustering$wc)
      }
      
      # 5. Alpha CHIC
      range.alpha <- seq(0, 1, by = 0.1)
      
      if(!is.null(spatial_clustering$d0) && !is.null(spatial_clustering$d1)) {
        
        # Panggil choicealpha
        cr <- choicealpha(
          spatial_clustering$d0,
          spatial_clustering$d1,
          range.alpha,
          K = 2,
          graph = FALSE,
          scale = TRUE
        ...
        spatial_clustering$alpha_results <- cr
        
        q0norm <- cr$Qnorm[,1]  # based on D0
        q1norm <- cr$Qnorm[,2]  # based on D1
        q1norm[q1norm > 1] <- 1
        
        alpha_df <- data.frame(
          Alpha = range.alpha,
          Q0norm = round(q0norm, 4),
          Q1norm = round(q1norm, 4)   
        )
        
        spatial_clustering$alpha_df <- alpha_df
      ...
      # 6. Spatial clustering dengan alpha = 0.3
      if(!is.null(spatial_clustering$d0) && !is.null(spatial_clustering$d1)) {
        alpha_used <- 0.3
        
        spatial_clustering$spatial_clust <- tryCatch({
          hclustgeo(spatial_clustering$d0, 
                    spatial_clustering$d1, 
                    alpha = alpha_used, 
                    scale = TRUE)
        }, error = function(e) {
          showNotification(paste("Error dalam hclustgeo:", e$message), type = "warning", duration = 3)
          NULL
        })
        
        # 7. Silhouette analysis
        max_k <- min(10, nrow(data_clean) - 1)
        if(max_k >= 2) {
          silhouette_avg <- numeric(max_k - 1)
          
          for(k in 2:max_k) {
            clusters <- cutree(spatial_clustering$spatial_clust, k = k)
            sil <- silhouette(clusters, dist = spatial_clustering$d0)
            silhouette_avg[k - 1] <- mean(sil[, 3])
          }
          
          spatial_clustering$silhouette_results <- data.frame(
            Jumlah_Klaster = 2:max_k,
            Average_Silhouette_Width = silhouette_avg
          )
          
          spatial_clustering$optimal_k <- spatial_clustering$silhouette_results$Jumlah_Klaster[
            which.max(spatial_clustering$silhouette_results$Average_Silhouette_Width)
          ]
          
          spatial_clustering$cluster_membership <- cutree(spatial_clustering$spatial_clust, 
                                                          k = spatial_clustering$optimal_k)
        } else {
          spatial_clustering$error_message <- "Jumlah observasi terlalu sedikit untuk silhouette analysis"
          showNotification(spatial_clustering$error_message, type = "warning", duration = 5)
        }
      }
      
      # Tandai sukses
      spatial_clustering$data_ready <- TRUE
      
      showNotification("Spatial clustering analysis completed successfully!", 
                       type = "default", duration = 3)
      
    }, error = function(e) {
      # Simpan error message
      error_msg <- paste("Error in spatial clustering:", e$message)
      spatial_clustering$error_message <- error_msg
      spatial_clustering$data_ready <- FALSE
      ...
      cat("\n=== SPATIAL CLUSTERING ERROR ===\n")
      cat(error_msg, "\n")
      cat("================================\n\n")
    })
  })
  
  # Output untuk menampilkan status
  output$spatial_clustering_status <- renderText({
    if(is.null(values$shape_data)) {
      return("Silakan upload file shapefile terlebih dahulu")
    }
    if(!is.null(spatial_clustering$error_message)) {
      return(paste("Error:", spatial_clustering$error_message))
    }
    if(!spatial_clustering$data_ready) {
      return("Sedang memproses data spatial clustering...")
    }
    return(paste("Spatial clustering siap | Optimal k =", spatial_clustering$optimal_k))
  })
  
  output$spatial_clustering_status_ui <- renderUI({
    if(is.null(values$shape_data)) {
      tags$div(
        style = "color: #7f8c8d; padding: 5px;",
        icon("info-circle"),
        " Belum ada data shapefile"
      )
    } else if(!is.null(spatial_clustering$error_message)) {
      tags$div(
        style = "color: #e74c3c; padding: 5px;",
        icon("exclamation-triangle"),
        " Error: ", spatial_clustering$error_message
      )
    } else if(!spatial_clustering$data_ready) {
      tags$div(
        style = "color: #f39c12; padding: 5px;",
        icon("spinner", class = "fa-spin"),
        " Memproses data spatial clustering..."
      )
    } else {
      tags$div(
        style = "color: #27ae60; padding: 5px;",
        icon("check-circle"),
        " Spatial clustering siap | Optimal k = ", spatial_clustering$optimal_k
      )
    }
  })
  
  # Output untuk mengecek jumlah neighbors
  output$neighbors_check <- renderPrint({
    req(spatial_clustering$data_ready, spatial_clustering$nb)
    
    n_neighbors <- card(spatial_clustering$nb)
    no_neighbors <- which(n_neighbors == 0)
    
    cat("NEIGHBORS CHECK\n")
    cat("===============\n\n")
    cat("Total regions:", length(n_neighbors), "\n")
    cat("Regions with no neighbors:", length(no_neighbors), "\n")
    
    if(length(no_neighbors) > 0 && !is.null(values$shape_data$Wlyh_rp)) {
      cat("\nRegions without neighbors:\n")
      for(i in no_neighbors) {
        cat("  -", values$shape_data$Wlyh_rp[i], "\n")
      }
    }
  })
  
  # OUTPUT: Moran's I Test
  output$moran_test_output <- renderPrint({
    req(spatial_clustering$data_ready)
    
    if(is.null(spatial_clustering$moran_test)) {
      cat("Moran's I test tidak dapat dihitung (mungkin karena masalah neighbors)")
      return()
    }
    ...
    cat("===========================================\n\n")
    cat("Data: Tingkat Kemiskinan (TngktKm)\n")
    cat("Weight matrix: Queen contiguity\n")
    cat("Zero.policy: TRUE (mengabaikan region tanpa neighbors)\n")
    cat("-------------------------------------------\n\n")
    
    print(spatial_clustering$moran_test)
  })
  ...
  output$moran_interpretation <- renderUI({
    ...
    if(is.null(spatial_clustering$moran_test)) {
      return(tags$div(
        style = "color: #e74c3c; padding: 15px; background-color: #fdeaea; border-radius: 5px;",
        icon("exclamation-triangle"),
        " Moran's I test tidak dapat dihitung. Kemungkinan ada region tanpa neighbors."
      ))
    }
    
    moran_stat <- spatial_clustering$moran_test$statistic[1]
    p_value <- spatial_clustering$moran_test$p.value
    moran_i <- spatial_clustering$moran_test$estimate[1]
    
    tagList(
      h4("Interpretasi Moran's I:", style = "color: #2c3e50;"),
      div(style = "background-color: #f8f9fa; padding: 15px; border-radius: 5px;",
          p(strong("Moran's I statistic:"), round(moran_i, 4)),
          p(strong("z-score:"), round(moran_stat, 4)),
          p(strong("p-value:"), format.pval(p_value, digits = 4)),
          hr(),
          h4("Kesimpulan:"),
          if(!is.na(p_value) && p_value < 0.1) {
            tags$div(
              style = "color: #e74c3c; font-weight: bold; padding: 10px; background-color: #fdeaea; border-radius: 5px;",
              icon("exclamation-triangle"),
              " Terdapat autokorelasi spasial yang signifikan pada α = 10%",
              br(),
              br(),
              if(moran_i > 0) {
                "Nilai positif mengindikasikan clustering spasial (wilayah dengan nilai kemiskinan serupa berkelompok)"
              } else {
                "Nilai negatif mengindikasikan dispersed pattern (wilayah dengan nilai berbeda cenderung bersebelahan)"
              }
            )
          } else {
            tags$div(
              style = "color: #27ae60; font-weight: bold; padding: 10px; background-color: #e8f5e9; border-radius: 5px;",
              icon("check-circle"),
              " Tidak terdapat autokorelasi spasial yang signifikan pada α = 10%",
              br(),
              br(),
              "Distribusi spasial tingkat kemiskinan bersifat acak (random pattern)"
            )
          }
      )
    )
  ...
  output$spatial_weight_summary <- renderPrint({
    req(spatial_clustering$data_ready)
    req(!is.null(spatial_clustering$w_listw))
    
    cat("\n")
    cat("===========================================\n")
    cat("      SPATIAL WEIGHT MATRIX SUMMARY\n")
    cat("===========================================\n\n")
    cat("Weight style: W (row-standardized)\n")
    cat("Neighbor definition: Queen contiguity\n")
    cat("-------------------------------------------\n\n")
    
    summary(spatial_clustering$w_listw)
  })
  
  # OUTPUT: Euclidean Distance Matrix
  output$euclidean_distance_matrix <- renderDT({
    req(spatial_clustering$data_ready)
    req(!is.null(spatial_clustering$d0))
    req(values$shape_data)
    
    dist_matrix <- as.matrix(spatial_clustering$d0)
    
    dist_df <- as.data.frame(round(dist_matrix, 4))
    
    if(!is.null(values$shape_data$Wlyh_rp)) {
      row_names <- values$shape_data$Wlyh_rp
      colnames(dist_df) <- row_names
      rownames(dist_df) <- row_names
    }
  ...
  output$distance_summary <- renderPrint({
    req(spatial_clustering$data_ready)
    req(!is.null(spatial_clustering$d0))
    
    dist_vec <- as.vector(spatial_clustering$d0)
    
    cat("\n")
    cat("===========================================\n")
    cat("   SUMMARY STATISTICS - EUCLIDEAN DISTANCES\n")
    cat("===========================================\n\n")
    cat("Number of distances:", length(dist_vec), "\n")
    cat("Min:", round(min(dist_vec), 4), "\n")
    cat("1st Quartile:", round(quantile(dist_vec, 0.25), 4), "\n")
    cat("Median:", round(median(dist_vec), 4), "\n")
    cat("Mean:", round(mean(dist_vec), 4), "\n")
    cat("3rd Quartile:", round(quantile(dist_vec, 0.75), 4), "\n")
    cat("Max:", round(max(dist_vec), 4), "\n")
    cat("Standard Deviation:", round(sd(dist_vec), 4), "\n")
  })
  ...
  output$distance_distribution <- renderPlot({
    ...
    hist(dist_vec, 
         ...
         col = "#3498db",
         border = "white",
         main = "Distribution of Euclidean Distances",
         xlab = "Distance",
         ylab = "Frequency",
         cex.main = 1.2)
    abline(v = median(dist_vec), col = "red", lwd = 2, lty = 2)
    abline(v = mean(dist_vec), col = "blue", lwd = 2, lty = 3)
    legend("topright", 
           legend = c(paste("Median =", round(median(dist_vec), 4)),
                      paste("Mean =", round(mean(dist_vec), 4))), 
           col = c("red", "blue"), 
           lty = c(2, 3), 
           lwd = 2)
  })
  
  # OUTPUT: Binary Weight Matrix
  output$binary_weight_matrix <- renderDT({
    req(spatial_clustering$data_ready)
    req(!is.null(spatial_clustering$wc))
    req(values$shape_data)
    
    wc_df <- as.data.frame(spatial_clustering$wc)
    
    if(!is.null(values$shape_data$Wlyh_rp)) {
      row_names <- values$shape_data$Wlyh_rp
      colnames(wc_df) <- row_names
      rownames(wc_df) <- row_names
    }
  ...
  # OUTPUT: Standardized Weight Matrix 
  output$standardized_weight_matrix <- renderDT({
    req(spatial_clustering$data_ready)
    req(!is.null(spatial_clustering$ww))
    req(values$shape_data)
    
    ww_df <- as.data.frame(round(spatial_clustering$ww, 4))
    
    if(!is.null(values$shape_data$Wlyh_rp)) {
      row_names <- values$shape_data$Wlyh_rp
      colnames(ww_df) <- row_names
      rownames(ww_df) <- row_names
    }
    ...
    datatable(ww_df, 
      ...
      formatStyle(
        ...
        backgroundColor = styleInterval(
          c(0, 0.0001),
          c('#ffffff', '#2ecc71', '#2ecc71')
        )
      )
  })
  
  output$weights_characteristics <- renderPrint({
    req(spatial_clustering$data_ready)
    req(!is.null(spatial_clustering$nb))
    req(!is.null(spatial_clustering$wc))
    
    cat("\n")
    cat("===========================================\n")
    cat("      SPATIAL WEIGHTS CHARACTERISTICS\n")
    cat("===========================================\n\n")
    cat("Number of regions (n):", length(spatial_clustering$nb), "\n")
    
    n_neighbors <- card(spatial_clustering$nb)
    cat("\nNeighbors distribution:\n")
    cat("  Minimum neighbors:", min(n_neighbors), "\n")
    cat("  Maximum neighbors:", max(n_neighbors), "\n")
    cat("  Mean neighbors:", round(mean(n_neighbors), 2), "\n")
    cat("  Median neighbors:", median(n_neighbors), "\n")
    
    cat("\nConnectivity:\n")
    cat("  Total links:", sum(n_neighbors), "\n")
    cat("  Density:", round(sum(n_neighbors)/(length(spatial_clustering$nb)^2)*100, 4), "%\n")
    
    cat("\nSparsity of weight matrix:\n")
    total_cells <- length(spatial_clustering$nb)^2
    nonzero_cells <- sum(spatial_clustering$wc != 0)
    sparsity <- (total_cells - nonzero_cells) / total_cells * 100
    cat("  Non-zero elements:", nonzero_cells, "\n")
    cat("  Zero elements:", total_cells - nonzero_cells, "\n")
    cat("  Sparsity:", round(sparsity, 2), "%\n")
  })
  
  # OUTPUT: Alpha CHIC
  output$alpha_choice_table <- renderDT({
    req(spatial_clustering$data_ready)
    
    if(is.null(spatial_clustering$alpha_df)) {
    ...
    datatable(spatial_clustering$alpha_df, 
              options = list(
                pageLength = 11, 
                dom = 't',
  ...
  output$alpha_choice_plot <- renderPlot({
    req(spatial_clustering$data_ready)
    req(!is.null(spatial_clustering$alpha_df))
    
    alpha_vals <- spatial_clustering$alpha_df$Alpha
    q_d0 <- spatial_clustering$alpha_df$Q0norm
    q_d1 <- spatial_clustering$alpha_df$Q1norm
    
    plot(alpha_vals, q_d0, 
         type = "b", pch = 19, col = "blue",
         xlab = "Alpha", ylab = "Qnorm",
         main = "Choice of Alpha - Normalized Criterion",
         ylim = c(0, 1.1),
         lwd = 2)
    
    lines(alpha_vals, q_d1, type = "b", pch = 17, col = "red", lwd = 2)
    
    grid()
    legend("topright", 
           legend = c("based on D0", "based on D1"),
           col = c("blue", "red"),
           pch = c(19, 17),
           lty = 1)
  })
  
  output$alpha_recommendation <- renderPrint({
    req(spatial_clustering$data_ready)
    
    cat("\n")
    cat("===========================================\n")
    cat("           PARAMETER ALPHA CHIC\n")
    cat("===========================================\n\n")
    cat("Tabel di atas menunjukkan nilai Qnorm berdasarkan atribut (D0) dan spasial (D1)\n")
    cat("Rekomendasi alpha = 0.3\n")
  })
  
  # OUTPUT: Silhouette
  output$silhouette_plot <- renderPlot({
    req(spatial_clustering$data_ready)
    req(!is.null(spatial_clustering$silhouette_results))
    req(!is.null(spatial_clustering$optimal_k))
    
    plot(spatial_clustering$silhouette_results$Jumlah_Klaster,
         spatial_clustering$silhouette_results$Average_Silhouette_Width,
         type = "b",
         pch = 19,
         col = "black",
         xlab = "Jumlah Klaster",
    ...
    abline(v = spatial_clustering$optimal_k, col = "red", lty = 2, lwd = 2)
    
    text(spatial_clustering$optimal_k, 
         max(spatial_clustering$silhouette_results$Average_Silhouette_Width) * 0.9,
         paste("Optimal k =", spatial_clustering$optimal_k),
  ...
  output$silhouette_table <- renderDT({
    ...
    df <- spatial_clustering$silhouette_results
    ...
    datatable(df,
              ...
              rownames = FALSE) %>%
      formatRound(columns = "Average_Silhouette_Width", digits = 4) %>%
      formatStyle(
        'Average_Silhouette_Width',
        backgroundColor = styleColorBar(df$Average_Silhouette_Width, '#2ecc71')
      )
  })
  
  # OUTPUT: Optimal Cluster Info 
  output$optimal_cluster_info <- renderUI({
    req(spatial_clustering$data_ready)
    req(!is.null(spatial_clustering$optimal_k))
    req(!is.null(spatial_clustering$silhouette_results))
    
    max_sil <- max(spatial_clustering$silhouette_results$Average_Silhouette_Width)
    optimal_k <- spatial_clustering$optimal_k
    
    tagList(
      div(style = "text-align: center; padding: 20px;",
          h4("Optimal Number of Clusters:", style = "color: #2c3e50;"),
          h2(optimal_k, style = paste0("color: #3498db; font-size: 72px; font-weight: bold;")),
          h4(paste("Average Silhouette Width =", round(max_sil, 4)), style = "color: #7f8c8d;"),
          hr(),
          div(style = "background-color: #f8f9fa; padding: 15px; border-radius: 10px; margin-top: 20px;",
              h4("Interpretasi:"),
              p("Semakin tinggi nilai silhouette, semakin baik pengelompokan wilayah")
          )
      )
    )
  })
  ...
  # OUTPUT: Dendrogram 
  output$spatial_dendrogram <- renderPlot({
    req(spatial_clustering$data_ready)
    req(!is.null(spatial_clustering$spatial_clust))
    req(!is.null(spatial_clustering$optimal_k))
    req(values$shape_data)
    
    # Plot dendrogram
    plot(spatial_clustering$spatial_clust,
         labels = values$shape_data$Wlyh_rp,
    ...
    # Cluster 1: cyan, Cluster 2: red
    rect.hclust(spatial_clustering$spatial_clust, 
                k = 2, 
                border = c("red", "blue"))
  ...
  output$cluster_result_map <- renderLeaflet({
    req(values$shape_data)
    req(spatial_clustering$data_ready)
    req(!is.null(spatial_clustering$cluster_membership))
    
    data <- values$shape_data
    data$cluster_result <- factor(spatial_clustering$cluster_membership)
    
    n_clusters <- length(unique(spatial_clustering$cluster_membership))
    
    if(n_clusters == 2) {
      cluster_colors <- c("cyan", "pink")
    } else {
    ...
    pal <- colorFactor(
      ...
      domain = data$cluster_result
    )
    
    leaflet(data) %>%
      addProviderTiles(providers$CartoDB.Positron) %>%
      addPolygons(
        fillColor = ~pal(cluster_result),
        fillOpacity = 0.7,
        color = "white",
        weight = 1,
        label = ~paste(Wlyh_rp, "- Cluster", cluster_result),
        ...
        popup = ~paste0(
          "<b>", Wlyh_rp, "</b><br/>",
          "Cluster: ", cluster_result, "<br/>",
          "Tingkat Kemiskinan: ", round(TngktKm, 2)
        )
      ) %>%
      addLegend(
        position = "bottomright",
        pal = pal,
        values = ~cluster_result,
  ...
  calculate_diagnostics <- function() {
    ...
    values$diagnostics$ks <- tryCatch({ks.test(residuals(model), "pnorm", mean = mean(residuals(model)), sd = sd(residuals(model)))}, error = function(e) NULL)
